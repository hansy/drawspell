...
export const Card: React.FC<CardProps> = ({ card, style: propStyle, className, onContextMenu, faceDown }) => {
    ...
    return (
        <div
            ...
            {...attributes}
            className={cn(
                "w-24 h-32 bg-zinc-800 rounded-lg border border-zinc-700 shadow-md flex flex-col items-center justify-center select-none hover:scale-105 hover:shadow-xl hover:z-10 hover:border-indigo-500/50",
                // Transitions disabled per user request
                // !isDragging && "transition-transform duration-300 ease-out",
                card.tapped && "rotate-90 border-zinc-600 bg-zinc-900",
