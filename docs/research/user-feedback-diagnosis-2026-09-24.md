...
- **Bulk counters and P/T (#5):** Bulk counters are implemented for a controlled battlefield selection. The group menu opens the add-counter dialog for all selected cards, including a count per card, or prompts for a counter type and count to remove from each card, capped at zero. The dialog shows how many cards it will affect. Bulk P/T adjustment remains open. P/T edits currently change card stats directly (`apps/web/src/lib/cardPT.ts`), so its future UI should describe their duration honestly rather than imply they expire at end of turn.
