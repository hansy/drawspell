...
- **Bulk counters and P/T (#5):** The group menu currently offers reveal, move, and optional token removal (`apps/web/src/models/game/context-menu/menu/groupActions.ts`). Add a group action that applies one signed counter adjustment to every eligible selected card, plus a distinct signed P/T adjustment. Show how many cards will change, respect per-card permissions, and make the operation consistent for all selected cards. P/T edits currently change card stats directly (`apps/web/src/lib/cardPT.ts`), so the UI should describe their duration honestly rather than imply they expire at end of turn.
