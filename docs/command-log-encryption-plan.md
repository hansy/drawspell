...
- `playerKey` is also used to compute a command MAC (HMAC) so spectators cannot submit valid commands.
...
```json
{
  ...
  "type": "card.move",
  "payloadPublic": { "...": "..." },
  "payloadOwnerEnc": "base64(aes-gcm)",
  "payloadSpectatorEnc": "base64(aes-gcm)",
  "payloadRecipientsEnc": { "playerId": "base64(aes-gcm)" },
...
- `mac` is required for state-mutating commands and is computed from `playerKey`.
...
- `sig` = Ed25519 signature of the canonical bytes.
- `mac` = HMAC-SHA256 over the same canonical bytes, using `playerKey`-derived MAC key.
...
- Include the ephemeral public key and nonce in the ciphertext blob.
...
```json
{
  ...
  "publicState": { "...": "..." },
  "ownerEncByPlayer": { "playerId": "base64(aes-gcm)" },
  "spectatorEnc": "base64(aes-gcm)",
  "pubKey": "base64(ed25519 public key)",
  "mac": "base64(hmac-sha256)",
  "sig": "base64(ed25519 signature)"
}
...
- AES-GCM, 96-bit nonce, base64 output.
- Hidden zones:
...
4. `mac` is valid for state-mutating commands (using `playerKey`).
...
- Periodically emit signed snapshots (every N commands).
- A snapshot contains:
...
1) **Crypto utilities**: add canonical JSON encoding, base64url helpers, SHA-256, HKDF, AES-GCM, Ed25519 + X25519 using `@noble/curves`, with unit tests (`apps/web/src/crypto/*`).
